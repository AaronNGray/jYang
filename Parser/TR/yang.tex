...
or delete it or some part of it. Configurations are represented in XML
...
A  \nc\ agent  can have  several configurations  each  ones containing
several   configuration  data.    There   can  be   only  one   active
configuration,  called the  {\tt running}  configuration, at  the same
time. Other configuration,  called {\tt candidate} configurations, can
exist  without  interfers  with  the  running one.  A  special  commit
...
\nc\  can  use  several  connexion-oriented transport  protocols.   It
requires  that  a persistent  connexion  is  maintained between  peers
...
released at the end of the connexion.

Authentication, integrity and confidentiality  must be provided bu the
...
copy config  operations originated from  an other session  or external
access (as SNMP);
\item
...
\item
{\tt <kill-session>} : to stop  the NETCONF session without achieving any
operations in progress.
\end{itemize}

All  operations are in  {\tt <rpc>} elements. A common element of
...
follow the \y\  syntax. \y\ provides constructs that  give semantic to
XML data. As  XML document is a collection  of imbricated markups, \y\
defines statements that can be  mapped on pattern of markups. Moreover
\y\  allows reusability  of specification  with generic  statements or
augmentation/extension statements.
